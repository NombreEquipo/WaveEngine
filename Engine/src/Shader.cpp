...
bool Shader::CreateLinesShader()
{
    ...
    LOG_CONSOLE("Line Shader created successfully!");
    return true;
}
...
bool Shader::CreateWater()
{
    ...
    const char* geometryShaderSource = "#version 330 core\n"
        ...
        "    \n"
        "    // Limit horizontal displacement to avoid self-intersection at high amplitudes\n"
        "    float overlapCheck = w.steepness * waveAmplitude;\n"
        "    float hScale = 1.0;\n"
        "    if (overlapCheck > 0.35) hScale = 0.35 / overlapCheck;\n"
        "    \n"
        "    float cosf = cos(f);\n"
        "    float sinf = sin(f);\n"
        "    \n"
